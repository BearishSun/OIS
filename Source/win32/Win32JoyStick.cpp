...
#define XINPUT_TRANSLATED_BUTTON_COUNT 16
...
//--------------------------------------------------------------------------------------------------//
void Win32JoyStick::initializeDirectInput(HWND hWnd)
{
	mHwnd = hWnd;
	...
	else
	{
		...
		if(FAILED(mJoyStick->SetCooperativeLevel( mHwnd, coopSetting)))
		...
		if( FAILED(mJoyStick->SetProperty(DIPROP_BUFFERSIZE, &dipdw.diph)) )
			OIS_EXCEPT( E_General, "Win32JoyStick::Win32Mouse >> Failed to set buffer size property" );
...
//--------------------------------------------------------------------------------------------------//
void Win32JoyStick::uninitializeDirectInput()
{
	delete mFfDevice;
...
//--------------------------------------------------------------------------------------------------//
void Win32JoyStick::setCaptureContext(const std::string& source)
{
	HWND hWnd;
...
void Win32JoyStick::captureXInput()
{
    ...
    for (size_t i = 0; i < XINPUT_TRANSLATED_BUTTON_COUNT; i++)
        mState.mButtons[i] = (inputState.Gamepad.wButtons & (1 << i)) != 0;
