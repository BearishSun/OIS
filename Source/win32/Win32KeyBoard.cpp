...
Win32Keyboard::Win32Keyboard( InputManager* creator, IDirectInput8* pDI, bool buffered, DWORD coopSettings )
	: Keyboard(creator->inputSystemName(), buffered, 0, creator), mActiveHWND(0)
{
	mKeyboard = 0;
	mDirectInput = pDI;
...
void Win32Keyboard::initializeDirectInput(HWND hWnd)
{
	...
	if(FAILED(mKeyboard->SetCooperativeLevel(hWnd, coopSetting)))
...
//--------------------------------------------------------------------------------------------------//
void Win32Keyboard::uninitializeDirectInput()
{
	if(mKeyboard)
	{
		mKeyboard->Unacquire();
...
//--------------------------------------------------------------------------------------------------//
void Win32Keyboard::setCaptureContext(const std::string& source)
{
	HWND hWnd;
	hWnd  = (HWND)strtoull(source.c_str(), 0, 10);

	if(hWnd != mActiveHWND)
	{
		if( IsWindow(hWnd) == 0 )
			OIS_EXCEPT( E_General, "Win32Keyboard::setCaptureContext >> The sent HWND is not valid!");
...
void Win32Keyboard::_readBuffered()
{
	...
	for(unsigned int i = 0; i < entries; ++i )
	{
		...
		if( diBuff[ i ].dwData & 0x80 )
		{
			...
			if( mListener )
				ret = mListener->keyPressed( KeyEvent( this, kc, diBuff[i].dwTimeStamp ) );
		}
		else
		{
			//Turn off modifier
			...
			//Fire off event
			if( mListener )
				ret = mListener->keyReleased( KeyEvent( this, kc, diBuff[i].dwTimeStamp ) );
		}
	...
	if(verifyAfterAltTab)
	{
		...
		for (unsigned i = 0; i < 256; i++)
		{
			if (keyBufferCopy[i] != KeyBuffer[i])
			{
				...
				{
					if (KeyBuffer[i])
						ret = mListener->keyPressed( KeyEvent( this, (KeyCode)i, GetTickCount()) );
					else
						ret = mListener->keyReleased( KeyEvent( this, (KeyCode)i, GetTickCount()) );
				}
			}
