...
//--------------------------------------------------------------------------------------------------//
void Win32Mouse::uninitializeDirectInput()
{
	if (mMouse)
	{
		mMouse->Unacquire();
...
//--------------------------------------------------------------------------------------------------//
void Win32Mouse::setCaptureContext(const std::string& source)
{
	HWND hWnd;
...
bool Win32Mouse::_doMouseClick( int mouseButton, DIDEVICEOBJECTDATA& di )
{
	if( di.dwData & 0x80 )
	{
		...
		if( mListener && mBuffered )
			return mListener->mousePressed( MouseEvent( this, mState, di.dwTimeStamp ), (MouseButtonID)mouseButton );
	}
	else
	{
		mState.buttons &= ~(1 << mouseButton); //turn the bit flag off
		if( mListener && mBuffered )
			return mListener->mouseReleased( MouseEvent( this, mState, di.dwTimeStamp ), (MouseButtonID)mouseButton );
	}
